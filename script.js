...
        if (item.medidasProcesadas && item.medidasProcesadas.length > 0) {
            medidasTexto = item.medidasProcesadas.map(medida => {
                const anchoVal = medida.ancho || 'N/A';
                const altoVal = medida.alto || 'N/A';
                // Aquí aplicamos la 'x' como pediste
                return `Ancho: ${anchoVal} x Alto: ${altoVal}`;
            }).join('<br>'); // Si hay múltiples, las separamos con un salto de línea
        }
        
