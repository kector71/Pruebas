...
                    if (manufacturerFilter === 'K') return refPart.startsWith('K');
                    if (manufacturerFilter === 'INC') return refPart.endsWith('INC');
                    if (manufacturerFilter === 'BP') return refPart.endsWith('BP');
                    if (manufacturerFilter === 'B') return refPart.endsWith('BEX');
                    return false;
                });
            }
...
IA     els.anio.value = params.get('anio') || '';
...
service   const debouncedFilter = debounce(filterData, 300);
...
      A     els.searchContainer.classList.toggle('active', e.target.value.trim() !== '');
...
s       if (els.clearBtn.disabled) return;
...
I             const safeOems = Array.isArray(item.oem) ? item.oem.map(String) : [];
...
Read                 if (app[prop]) allValues.add(String(app[prop]));
                    });
                });
s             return [...allValues].sort();
            };
...
What       const allOems = [...new Set(appState.data.flatMap(i => i.oem || []))].filter(Boolean).sort();
s         const allFmsis = [...new Set(appState.data.flatMap(i => i.fmsi || []))].filter(Boolean).sort();
            fillDatalist(els.datalistOem, allOems);
            fillDatalist(els.datalistFmsi, allFmsis);

Ind       const allBrandsList = appState.data.flatMap(item => item.aplicaciones.map(app => app.marca)).filter(Boolean);
            const brandFrequencies = allBrandsList.reduce((counts, brand) => {
                counts[brand] = (counts[brand] || 0) + 1;
s             return counts;
            }, {});
...
            brandColorMap = {};
    key     allUniqueBrandsSorted.forEach((brand, index) => {
